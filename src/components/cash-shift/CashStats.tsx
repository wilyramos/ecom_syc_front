        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {cards.map((card) => (
                <div 
                    ...
                    className={
                        card.highlight 
                            ? "p-5 border border-[var(--color-accent)] bg-[var(--color-accent-light)] text-[var(--color-text-primary)] rounded-md transition-all" 
                            : "p-5 border border-[var(--color-border-default)] bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] rounded-md transition-all"
                    }
                >
                    <div className="flex items-center justify-between mb-4">
                        <span className={
                            card.highlight 
                                ? "text-[10px] font-black uppercase tracking-widest text-[var(--color-accent-hover)]" 
                        ...
                        <card.icon 
                            size={16} 
                            className={card.highlight ? "text-[var(--color-accent-hover)]" : "text-[var(--color-text-secondary)]"} 
                        />
                    </div>
                    <div className="flex items-baseline gap-1">
                        <span className={
                            card.highlight 
                                ? "text-sm font-medium text-[var(--color-accent-hover)] opacity-70" 
                                : "text-sm font-medium text-[var(--color-text-tertiary)]"
                        }>
                            S/
                        </span>
                        <p className={card.highlight ? "text-2xl font-mono font-bold tracking-tighter text-[var(--color-accent-hover)]" : "text-2xl font-mono font-bold tracking-tighter"}>
