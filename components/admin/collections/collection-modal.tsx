...
export default function CollectionModal({ isOpen, onClose, collectionToEdit }: Props) {
    ...
    // NUEVO: Estado para el color (Debe empezar con '#' y tener 7 caracteres para el input tipo color)
    const [selectedColor, setSelectedColor] = useState<string>(
        collectionToEdit?.color && /^#[0-9A-F]{6}$/i.test(collectionToEdit.color)
            ? collectionToEdit.color
            : "#ffffff"
    );
            ...
            <DialogContent className="sm:max-w-lg bg-background border border-border rounded-sm shadow-xs outline-none">
                ...
                <form action={formAction} className="space-y-4 pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <Label>Color de la colección</Label>
                            <div className="flex gap-2 items-center">
                                {/* Selector visual interactivo */}
                                <div className="relative w-10 h-10 rounded-md border border-border overflow-hidden shrink-0 subtle-shadow">
                                    <input
                                        type="color"
                                        value={selectedColor}
                                ...
                                {/* Input de texto que se envía en el Form Action */}
                                <Input
                                    name="color"
                                    value={selectedColor}
                                    onChange={(e) => setSelectedColor(e.target.value)}
