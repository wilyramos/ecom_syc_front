    ...
    const content = (
        ...
        >
            <div className="relative z-10 w-full max-w-6xl mx-auto h-full flex flex-row items-center ">
                ...
                {/* ── Texto (derecha) ───────────────────────────────── */}
                <div
                    className="flex flex-col justify-center items-start text-left w-1/2 h-full pl-3 sm:pl-6 gap-1 md:gap-3"
                    style={{ color: text }}
                >
                    {subtitle && (
                        <div>
                    ...
                    {description && (
                        <p
                            className="text-[9px] sm:text-xs md:text-sm leading-tight sm:leading-relaxed line-clamp-2 md:line-clamp-3 max-w-[40ch]"
                            style={{ opacity: 0.75 }}
                        >
                            {description}
                        </p>
                    ...
                    {price?.current !== undefined && price.current !== null && (
                        <div className="mt-0.5 scale-90 sm:scale-100 origin-left shrink-0">
                            <SliderPrice
                                price={price}
                                textColor={text}
                                accentColor={accent}
