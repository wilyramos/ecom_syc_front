...
export default function LayoutImageCenterSplit({ banner }: { banner: SliderBanner }) {
            ...
            case "title":
                return title && (
                    <div key="title" style={fadeSlide(animationSide, delay)} className="flex-none max-w-full">
                        <h2
                            className="font-bold leading-tight tracking-tight text-[clamp(1.1rem,2vw,1.8rem)] line-clamp-3"
                            style={{ color: text }}
                        >
                            {title}
                        </h2>
    ...
    return (
        <Link
            ...
            className="banner-slot group relative block w-full overflow-hidden h-[var(--banner-h-mobile)] md:h-[var(--banner-h)]"
            style={{ backgroundColor: bg }}
        >
            {/* ── Desktop (≥ sm): Contenido centrado y restringido ── */}
            <div className="hidden sm:flex relative z-20 max-w-6xl mx-auto h-full w-full">
                
                {/* Panel izquierdo */}
                <div
                    className="h-full flex flex-col items-center justify-center text-center gap-3 md:gap-4 flex-shrink-0 z-20 overflow-hidden"
                    style={{ width: "28%", paddingInline: "2%" }}
                >
                    {leftFields.map((f, i) => renderField(f, "left", i))}
                </div>

                {/* Imagen central: Se ajusta al espacio restante dentro del max-w-6xl */}
            ...
            {/* ── Mobile (< sm): Comportamiento original (sangre completa) ── */}
