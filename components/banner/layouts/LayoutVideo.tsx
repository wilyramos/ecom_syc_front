...
export default function LayoutVideo({ banner }: { banner: SliderBanner }) {
    ...
    const accent = design.accentColor ?? (isDark ? "#00CDF7" : "#00CDF7");
    ...
    return (
        <Link
            ...
            /* Contenedor raíz con fondo a ancho completo y altura dinámica */
            className="banner-slot group relative w-full overflow-hidden flex items-center justify-center text-center h-[var(--banner-h-mobile)] md:h-[var(--banner-h)]"
        ...
        >
            ...
            {/* Overlays de contraste (Fondo absoluto) */}
            <div
            ...
            {/* ── Capa de Contenido: Restringida a max-w-6xl y centrada ── */}
            <div className="relative z-20 w-full max-w-6xl mx-auto h-full flex flex-col items-center justify-center px-6">

                <div
                    className="flex flex-col items-center w-full max-w-[22rem] sm:max-w-xl md:max-w-2xl lg:max-w-4xl"
                    style={{ color: text }}
                >
                    {subtitle && (
                        <div style={fadeUp(0.1)} className="mb-2">
                            <span
                                className="inline-block text-[10px] font-bold tracking-[0.4em] uppercase px-4 py-1.5 rounded-full backdrop-blur-md"
                                style={{
                                    color: text,
                    ...
                    {title && (
                        <h2
                            className="font-black leading-[1.05] tracking-[-0.05em] text-[clamp(1.8rem,5vw,3rem)] mt-2 mb-4 uppercase"
                            style={fadeUp(0.2, {
                                transform: loaded
